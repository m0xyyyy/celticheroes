...
function renderDossier(m){$('#emptyState').classList.add('hidden');const d=$('#dossier');d.classList.remove('hidden');
 ...
 <div class="section"><div class="section-head"><div class="section-title">Combat profile</div></div><div class="cards"><div class="panel"><h3>Core values</h3><div class="kv">${kv('Experience',fmt(m.experience))}${kv('Gold dropped',m.gold)}${kv('Ignore invisibility',`${fmt(m.ignoreInvis*10)}% chance`)}${kv('AI type',fmt(m.aiType))}${kv('Attack / missile',esc(m.rangeData))}${kv('Database field',fmt(m.field21))}</div></div><div class="panel"><h3>Resistances</h3><div class="chips">${Object.entries(m.resist).map(([k,v])=>`<span class="chip">${esc(k)} <b>${esc(fmt(v))}</b></span>`).join('')}</div></div></div></div>
...
function stat(k,v,u=''){return `<div class="stat"><div class="k">${k}</div><div class="v">${esc(fmt(v))}</div>${u?`<small>${u}</small>`:''}</div>`}function kv(k,v){return `<div><div class="k">${k}</div><div class="v">${v}</div></div>`}
...
function openSpawns(m,spawns){const zmap={};state.zones.forEach(z=>zmap[z.id]=z);const grouped=defaultdict(spawns,s=>s.zone);let first=spawns[0],zone=zmap[first.zone],map=`maps/${zone?.map||''}.jpg`;$('#modal').classList.remove('hidden');$('#modalBody').innerHTML=`<div class="eyebrow">SPAWN DATA</div><h2 style="margin:0 0 5px">${esc(m.name)}</h2><p class="muted">${spawns.length} spawn point${spawns.length===1?'':'s'} across ${new Set(spawns.map(s=>s.zone)).size} zone${new Set(spawns.map(s=>s.zone)).size===1?'':'s'}.</p><div class="spawn-grid">${spawns.slice(0,24).map((s,i)=>{const zz=zmap[s.zone];return `<div class="spawn-card" data-spawn-index="${i}" style="cursor:pointer"><b>${esc(zz?.name||`Zone ${s.zone}`)}</b><span>Spawn ${s.id} • ${esc(s.respawn)} • ${s.behavior}</span><div style="margin-top:6px;font-size:10px;color:#c8bcff">X ${s.x.toFixed(2)} • Y ${s.y.toFixed(2)} • Z ${s.z.toFixed(2)}</div></div>`}).join('')}</div><div class="map-wrap"><img id="spawnMap" src="${map}" alt="Zone map" onerror="this.style.display='none'"><div id="mapDots"></div></div>`;renderMapDots(spawns,zone);$$('[data-spawn-index]').forEach(el=>el.onclick=()=>{const s=spawns[Number(el.dataset.spawnIndex)];zone=zmap[s.zone];$('#spawnMap').src=`maps/${zone?.map||''}.jpg`;renderMapDots(spawns.filter(x=>x.zone===s.zone),zone)});}
function defaultdict(arr,key){const o={};arr.forEach(x=>(o[key(x)]??=[]).push(x));return o}
function renderMapDots(spawns,zone){const box=$('#mapDots');if(!box)return;box.innerHTML='';if(!zone)return;const w=zone.maxX-zone.minX,h=zone.maxZ-zone.minZ;spawns.forEach(s=>{const d=document.createElement('div');d.className='spawn-dot';d.style.left=((s.x-zone.minX)/w*100)+'%';d.style.top=((s.z-zone.minZ)/h*100)+'%';d.title=`Spawn ${s.id}`;box.appendChild(d)})}
