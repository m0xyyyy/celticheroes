...
function renderDossier(m){$('#emptyState').classList.add('hidden');const d=$('#dossier');d.classList.remove('hidden');
 ...
 <div class="section"><div class="section-head"><div class="section-title">Combat profile</div></div><div class="cards"><div class="panel"><h3>Core values</h3><div class="kv">${kv('Experience',fmt(m.experience))}${kv('Gold dropped',m.gold)}${kv('Ignore invisibility',`${fmt(m.ignoreInvis*10)}% chance`)}${kv('Attack speed', (m.rangeData||'').toString().split(/[,;]/)[0]||'—')}</div></div><div class="panel"><h3>Resistances</h3><div class="chips">${Object.entries(m.resist).map(([k,v])=>`<span class="chip">${esc(k)} <b>${esc(fmt(v))}</b></span>`).join('')}</div></div></div></div>
...
function renderMapDots(spawns,zone){const box=$('#mapDots');if(!box)return;box.innerHTML='';if(!zone)return;const w=zone.maxX-zone.minX||1,h=zone.maxZ-zone.minZ||1;spawns.forEach(s=>{const d=document.createElement('div');d.className='spawn-dot';const px=((s.x-zone.minX)/w*100);const py=((s.z-zone.minZ)/h*100);d.style.left=px+'%';d.style.top=py+'%';d.title=`Spawn ${s.id}\nX ${s.x.toFixed(2)}  Y ${s.y.toFixed(2)}  Z ${s.z.toFixed(2)}`;box.appendChild(d)})}
